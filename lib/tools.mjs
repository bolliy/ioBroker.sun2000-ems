...
async function getSystemData(adapter) {
	...
	if (state) {
		adapter.longitude = state.common.longitude;
		adapter.latitude = state.common.latitude;
		adapter.log.info(`system longitude ${adapter.longitude} latitude ${adapter.latitude}`);
	}
}
...
class Now {
	...
	constructor(adapterInstance) {
		...
		this._nowIdx2 = (this._now.getHours() * 4 + this._now.getMinutes() / 15) >> 0;
	}
	...
	renew(referenceTime) {
		...
		this._nowIdx2 = (this._now.getHours() * 4 + this._now.getMinutes() / 15) >> 0;
		//nächster Tag
		if (this._now.getDate() != dt.getDate()) {
			this._nowIdx += 24;
			this._nowIdx2 += 96; // 96 = 24 * 4
		}
	}
...
export { getAstroDate, adapterAlive, toDateString, isNumber, getStateValue, Average, Now, getSystemData };
