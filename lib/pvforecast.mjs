...
class PVForecast {
	/**
	 * Constructs a new instance of the PVForecast class.
	 *
	 * @param {object} adapterInstance - The ioBroker adapter instance.
	 * @param {string} [intanceName=''] - The name of the instance to be used for PV forecasting.
	 * @param {number} [probabilityFactor=0.75] - The factor representing the probability of PV forecast accuracy.
	 */

	constructor(adapterInstance, intanceName = '', probabilityFactor = 0.75) {
		...
		for (let h = 5; h < 22; h++) {
			...
			if (state) {
				if (tools.isNumber(state.val)) {
					pushItem(h, Math.round(state.val * factor * this.probabilityFactor), dt); //pushItem(h, state.val * factor * this.probabilityFactor, dt);
				}
				//pushItem(h, 0 ,dt);
			}
		}
