...
class Sun2000 {
	...
	async isBatteryRunning() {
		const status = await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.battery.runningStatus'); //await this.adapter.getState('sun2000.0.inverter.0.battery.runningStatus')?.val;
	...
	async getSurplusMinSoc() {
		return await tools.getStateValue(this.adapter, 'sun2000.0.control.usableSurplus.minSoc'); //await this.adapter.getState('sun2000.0.control.usableSurplus.minSoc')?.val;
	}
}
