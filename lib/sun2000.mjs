...
class Sun2000 {
	...
	async init() {
		this.ratedCapacity = await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.battery.ratedCapacity');
		this.maximumChargePower = await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.battery.maximumChargePower');
		await this.getDischargeCutoffCapacity();
		this.chargingCutoffCapacity = await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.battery.chargingCutoffCapacity');
	}
	...
	async setChargeFromGridFunction(value) {
		if (value === undefined) {
			const first = this.pullValue('chargeFromGrid');
			value = first;
		}
		if (value !== undefined) {
			...
			this.pushValue('chargeFromGrid', value);
		}
	}

	async getChargeDischargePower() {
		return await tools.getStateValue(this.adapter, 'sun2000.0.collected.chargeDischargePower');
	}
	...
	async getDeviceStatus() {
		return await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.deviceStatus');
	}

	async wakeUp() {
		this.setChargeFromGridFunction(true);
		const deviceStatus = await this.getDeviceStatus();
		//Shutdown: End of the ESS discharge process || Shutdown: command
		if (deviceStatus === 0x030c || deviceStatus === 0x0301) {
			await this.adapter.setForeignState('sun2000.0.inverter.0.control.startup', { val: true });
		}
	...
	async stopCharging() {
		try {
			...
			await this.adapter.setForeignState('sun2000.0.inverter.0.control.battery.forcibleChargePower', { val: 0 });
	...
	async isChargingFromGrid() {
		const val = await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.control.battery.forcibleChargeOrDischarge');
		const chargePower = await this.getChargeDischargePower();
		const setPoint = await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.control.battery.forcibleChargePower');
		const ret = val === 1 && setPoint !== 0 && chargePower * 1000 > setPoint * 0.7; //70 %
		return ret;
	}
	...
	/**
	 * Set the maximum charging power for the battery.
	 * If unlock is true, set it to the maximum charging power.
	 * If unlock is false, set it to 0.
	 * @param {boolean} unlock - Whether to unlock or lock charging.
	 * @returns {Promise<boolean>} true if charging power was set, false otherwise.
	 */
	async setCharging(unlock) {
		let value;
		try {
			const power = await tools.getStateValue(this.adapter, 'sun2000.0.inverter.0.battery.maximumChargingPower');
			if (unlock) {
	...
	async getSurplusBufferSoc() {
		const value = await tools.getStateValue(this.adapter, 'sun2000.0.control.usableSurplus.bufferSoc');
	...
	async setSurplusMinSoc(value) {
		if (value > 100) value = 100;
		if (value < 0) value = 0;
		try {
			const soc = await tools.getStateValue(this.adapter, 'sun2000.0.control.usableSurplus.minSoc');
			if (value !== soc) {
				this.adapter.log.debug(`set minSOC to ${value} %`);
				await this.adapter.setForeignState('sun2000.0.control.usableSurplus.minSoc', { val: value });
			}
			return true;
		} catch {
			return false;
