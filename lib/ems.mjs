    ...
    */
	...
	_calcualteForecastBasedCharging() {
		...
		if (surplusRest >= 1000) {
			for (let i = this.now.idx; i < 12; i++) {
				if (this.load[i].pv > 0 && !this.load[i].chargedEnergy && this.tbl[i]?.soc >= minSoc) {
	...
	async forcastBasedCharging() {
		...
		for (let i = this.now.idx; i < this.load.length; i++) {
			if (this.tbl[i].soc < smallestSoc) smallestSoc = this.tbl[i].soc;
		}
		//this.adapter.log.debug(`forcastBasedCharging smallest SOC ${smallestSoc}`);

		const overSoc = Math.round(smallestSoc - minSoc);
		this.adapter.log.debug(`forcastBasedCharging overSOC ${overSoc}`);
		if (overSoc > 0) {
			minSoc = this.SOC - overSoc;
			this.adapter.log.debug(`forcastBasedCharging new minSOC ${minSoc}`);
			await this.inverter.setSurplusBufferSoc(minSoc);
			const tbl = this.iBattery.getSOCTable(this.load, this.load.length /* this.iPrices.jsonData.length */, minSoc);
