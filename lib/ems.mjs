...
class EMS {
    ...
    */
	...
	async forcastBasedCharging() {
		...
		//Simulierte die SocTable mit minSoc
		tbl = this.iBattery.getSOCTable(this.load, this.load.length, minSoc, true);
		//Ermittlung den größten SOC lt. Simulation
		...
		let suplusMinSoc = (surplusBufferSOC - minSoc) / 2 + minSoc;
		await this.inverter.setSurplusMinSoc(suplusMinSoc);
		if (surplusBufferSOC < suplusMinSoc) surplusBufferSOC = 0;
