import PVForcast from './pvforecast.mjs';
import LoadTable from './loadtable.mjs';
//import Tipper from './tipper.mjs';
...
class EMS {
	constructor(adapterIntance) {
		...
		//const homeId = this.adapter.getState('0_userdata.0.tibber.homeId')?.val;
		//this.iPrices = new Tipper(homeId);
		this.iPVForcast = new PVForcast(this.adapter, this.adapter.config.instancePvforecast);
	...
	//sucht Surplus in der SOCTable
	_getSurplusToday(start = 7) {
		let surplus = 0;
		let toIdx = 24;
		let startIdx = 0;
		...
		for (let i = start; i < toIdx; i++) {
			if (this.tbl[i]?.grid < 0) {
				...
				if (i >= this.now.idx) surplus -= this.tbl[i].grid;
			}
		}
		return { idx: startIdx, surpusToday: surplus };
	}
    ...
    */
	...
	_searchHighPrices(soc) {
		//console.log('D1 '+JSON.stringify(tbl));
	...
	_calcualteForecastBasedCharging() {
		//load soc table
		this.tbl = this.iBattery.getSOCTable(this.load, 47 /* this.iPrices.jsonData.length */, this.SOC);
		const surplus = this._getSurplusToday(12);
		this.adapter.log.debug(`Surplus Today: ${JSON.stringify(surplus)}`);
		let surplusRest = surplus.surpusToday * 0.75; //Ausfallwagnis

		if (surplusRest >= 1000) {
			for (let i = this.now.idx; i < 12; i++) {
				if (this.load[i].pv > 0 && !this.load[i].chargedEnergy && this.tbl[i]?.soc > 30) {
	...
	async optimizeCharging() {
        ...
        */
		...
		this._calcualteForecastBasedCharging();

		this.tbl = this.iBattery.getSOCTable(this.load, 47 /*this.iPrices.jsonData.length*/, this.SOC);
		//Preise
		/* Tibber
		for (let i = 0; i < this.tbl.length; i++) {
			this.tbl[i].price = this.prices[i].total;
		}
		*/
	...
	async batteryProtection() {
		...
		if (this.averageChargePower.value > this.inverter.maximumChargePower - 500 && this.averageConsumption.value > this.inverter.maximumChargePower) {
			...
			//Hohe Last fällt weg
			if (!this.tbl[this.now.idx]?.lockDischarging) {
	...
	async processOn() {
		...
		this.now.renew(this.iLoad.jsonData[0].time);
		await this.inverter.setDisCharging(this.tbl[this.now.idx]?.lockDischarging ?? false);
	}
	 ...
	 */
	async processForcastBasedCharging() {
		this.adapter.log.debug(
			`idx ${this.now.idx}  SOC ${this.SOC} tbl.soc ${this.tbl[this.now.idx]?.soc}  lockCharging ${this.tbl[this.now.idx]?.lockCharging}`,
		);
		if (this.tbl[this.now.idx]?.lockCharging) {
			let charge = this.SOC > 30 && this.SOC >= this.tbl[this.now.idx]?.soc;
			await this.inverter.setCharging(charge);
		} else {
			let charge = true;
			if (this.SOC >= 85) {
				const energy = ((this.inverter.chargingCutoffCapacity - this.SOC) * this.inverter.ratedCapacity) / 100;
				const surplus = this._getSurplusToday(this.now.idx).surpusToday * 0.75;
				if (energy < surplus) charge = false;
				this.adapter.log.debug(`processForcastBasedCharging energy ${energy} < surplus ${surplus} charge ${charge}`);
			}
			await this.inverter.setCharging(charge);
		}
	}
	...
	async dataPolling() {
		...
		this.loopHandle = setInterval(async () => {
			...
			this.processForcastBasedCharging();
