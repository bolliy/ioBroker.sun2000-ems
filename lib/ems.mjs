...
class EMS {
    ...
    */
	_lockDisCharging(highRange) {
		...
		const sum = { energy: 0, start: 0 }; //relevanten Ladepunkte

		for (let k = 0; k < highRange.length; k++) {
			sum.energy += highRange[k].energy;
			if (highRange[k].start > sum.start) sum.start = highRange[k].start; //spätester Startpunkt
		}
		//Preise sortieren
		const sort = [];
		//statet frühestens in der nächste viertel Stunde
		for (let i = this.now.idx2 + 1; i < sum.start; i++) {
			sort.push(prices[i]);
		}

		//Sortiert günstig zu teuer
		sort.sort(function (a, b) {
			return a.total - b.total;
		});

		//this.adapter.log.info('_lockDisCharging '+JSON.stringify(sort));
		let gridSum = 0;
		for (let k = 0; k < highRange.length; k++) {
			//Preise durchlaufen
			for (let i = 0; i < sort.length; i++) {
				if (sort[i].total < highRange[k].threshold && sort[i].idx < highRange[k].start) {
					const hh = (sort[i].idx / 4) >> 0; //zu welcher Stunde
					if (!this.load[hh].LockDisCharging || this.load[hh].LockDisCharging <= 0.75) {
						...
						if (dischargedEnergy > 50) {
							//nur ein Eintrag pro Zeitpunkt (Viertelstunde)
							const exist = lock.findIndex(x => x.idx === sort[i].idx);
							if (exist === -1) {
								if (!this.load[hh].LockDisCharging) this.load[hh].LockDisCharging = 0;
								this.load[hh].LockDisCharging += 0.25; // + 1/4 Stunde
		...
		this.adapter.log.debug(`_lockDisCharging ${gridSum}W von ${sum.energy}W ${JSON.stringify(lock)}`);
		return lock;
	}
	...
	_searchHighPrices(soc, chargingLossFactor = 1) {
		...
		for (let k = this.now.idx2 + 1; k < prices.length; k++) {
			if (smallestPrice === undefined || smallestPrice > prices[k].total) smallestPrice = prices[k].total;
			//durch Ladeverluste erhöht sich die obere Preisgrenze
			const priceFactor = 1 + (this.iBattery.chargingLosses / 100) * chargingLossFactor;
			...
			if (tbl[hh].grid > 0 && prices[k].total > threshold) {
				...
				const threshold = Math.round((prices[k].total / priceFactor) * 1000) / 1000;
				const chargingEnergy = Math.round((tbl[hh].grid / 4) * priceFactor); //+17% mehr Energie laden
				ret.push({
					threshold: threshold,
					price: prices[k].total,
					total: chargingEnergy * prices[k].total,
	...
	_putInLowZone(highRange, soc) {
		...
		for (let k = 0; k < highRange.length; k++) {
			...
			for (let i = 0; i < relLP.length; i++) {
				let energy = rest;
				//energy = power / 4
				if (energy > this.inverter.maximumChargePower / 4) energy = this.inverter.maximumChargePower / 4; //1250 Wh
				...
				if (energy > 0) {
					...
					if (exist > -1) {
						if (loadPoint[exist].power / 4 + energy > this.inverter.maximumChargePower / 4) {
							energy = this.inverter.maximumChargePower / 4 - loadPoint[exist].power / 4;
		...
		for (let i = 0; i < loadPoint.length; i++) {
			if (loadPoint[i].power < 50) {
				const hh = (loadPoint[i].idx / 4) >> 0;
				this.load[hh].chargedEnergy -= loadPoint[i].power / 4;
	...
	async forcastBasedCharging() {
		...
		if (suplusMinSoc > this.inverter.chargingCutoffCapacity) suplusMinSoc = this.inverter.chargingCutoffCapacity - 3;
	...
	async optimizeCharging() {
		...
		let highPrices = this._searchHighPrices(this.SOC, 0.5);
		this.lock = this._lockDisCharging(highPrices);
