...
class EMS {
	...
	_getSurplusToday(start = 7) {
		...
		for (let i = start; i <= toIdx; i++) {
			if (this.tbl[i]?.consumption) {
				if (startIdx === 0) startIdx = i;
				surplus += this.tbl[i]?.pv - this.tbl[i]?.consumption;
			}
			if (this.tbl[i]?.grid < 0) {
				if (i >= this.now.idx) feedIn -= this.tbl[i].grid;
			}
		}
		...
		*/
		if (this.tbl[this.now.idx]?.lockCharging) {
			...
			if (this.SOC >= 85 && this.SOC < this.inverter.chargingCutoffCapacity) {
				...
				const surplus = this._getSurplusToday(this.now.idx + 1)?.surplus * 0.75;
