...
class EMS {
    ...
    */
	...
	async forcastBasedCharging() {
		//Simulation
		//Justiert den Soc-Verlauf (SocTable) so weit nach unten, dass dischargeCutoffCapacity nicht unterschritten wird.
		//
		let minSoc = this.inverter.dischargeCutoffCapacity; //mindestens auf Entladegrenze setzen
		let bufferSOC = 0;
		let consum = 0;
		let pv = 0;
		//Soc-Table mit negativen SOC Werten
		...
		//Simulierte eine Soc-Table mit ninSoc
		tbl = this.iBattery.getSOCTable(this.load, this.load.length, minSoc, true);
		if (this.debug === 2) {
		...
		//Surplus Werte für den Adapter sun2000 setzen
		await this.inverter.setSurplusMinSoc(tbl[this.now.idx].soc);
		//PufferSOC auf minSoc + 10% setzen
		bufferSOC = tbl[this.now.idx].soc + 10;
		if (bufferSOC > this.inverter.chargingCutoffCapacity) bufferSOC = 0; //kein BufferSOC
		await this.inverter.setSurplusBufferSoc(bufferSOC);

		//Laden erlauben oder nicht
	...
	async controlCharging(restTime) {
		...
		if (this.SOC < this.process.soc) {
			const chargePower = await this.inverter.getChargeDischargePower();
			if (chargePower <= 0) {
	...
	async controlCharging2(task) {
		const duration = task.end.getTime() - this.now.date.getTime();
		const power = this.adjustPower(this.SOC, duration);
		//
		if (this.SOC < task.chargePoint?.soc) {
			const isCharge = await this.inverter.isChargingFromGrid();
			if (!isCharge) {
				this.adapter.log.debug(`### Lädt der Speicher gerdade ? ${await this.inverter.isChargingFromGrid()} ###`);
				this.adapter.log.info('Start grid charging ...');
	...
	async processeing2() {
		...
		//[{"idx":14,"time":"2025-10-24 14:00:00","charge":2314,"price":0.1973,"start":42,"soc":95}
		/*
		if (this.newChargingPoints.length === 0) {
			this.newChargingPoints.push({
				time: '2025-10-24 16:45:00',
				charge: 1000,
				soc: 75,
			});
		}
		*/
		//aktuellen ChargePoint suchen
		for (let i = 0; i < this.newChargingPoints.length; i++) {
			...
			if (this.now.date >= wakeUp && this.now.date < end) {
				...
				this.process.soc = task.chargePoint.soc;
			}
		}
		...
		if (Object.keys(task).length !== 0) {
			...
			if (await this.inverter.wakeUp()) {
				...
				if (this.now.date >= task.start && this.now.date < task.end) {
					await this.controlCharging2(task);
				}
			} else {
				this.adapter.log.debug('Waiting for woke up ...');
			}
		} else {
			if (await this.inverter.isChargingFromGrid()) {
				if (await this.inverter.stopCharging()) {
					this.adapter.log.info('charging stopped');
				}
			}
			//this.adapter.log.debug('gerade kein Charge Task aktive');
			//ggf. Charging beenden
		}
	}
	...
	async isHighBatteryLoad() {
		...
		if (this.averageChargePower.value < -this.inverter.maximumChargePower * 0.7 && this.averageConsumption.value > this.inverter.maximumChargePower * 0.5) {
	...
	async dataPolling() {
		...
		}, 300000); //1000*60*5 Minuten = 300000 - test: 10000
