...
class EMS {
			...
			for (let i = 0; i < relLP.length; i++) {
				...
				*/
				...
				if (!this.load[hh].LockDisCharging || this.load[hh].LockDisCharging <= 0.75) {
					let dischargedEnergy = Math.round((this.load[hh].consumption - this.load[hh].pv) / 4);
					if (tbl[hh]?.soc <= this.inverter.dischargeCutoffCapacity) dischargedEnergy = 0;
					if (dischargedEnergy > rest) dischargedEnergy = rest;
					if (dischargedEnergy > 0) {
	...
	_putInLowZone(highRange, soc) {
		...
		for (let k = 0; k < highRange.length; k++) {
			const relLP = []; //relevante Ladepunkte
			for (let i = this.now.idx2 + 1; i < highRange[k].start; i++) {
				const standbyFactor = 1 + (highRange[k].start - i) / 800; //Stanby-By Verluste ca. +0,5% pro Stunde
				//günstiger als die untere Preisschwelle
				if (prices[i].total < highRange[k].threshold / standbyFactor) {
					relLP.push({ idx: i, price: prices[i].total, standbyFactor: standbyFactor });
				}
			}
			//relevante Ladepunkte nach Einstandspreis aufsteigend sortieren
			relLP.sort(function (a, b) {
				return a.price * a.standbyFactor - b.price * b.standbyFactor;
			});
			...
			for (let i = 0; i < relLP.length; i++) {
				...
				let energy = Math.round(rest * relLP[i].standbyFactor);
				...
				if (energy > 0) rest -= Math.round(energy / relLP[i].standbyFactor);
				if (rest <= 0) break;
			}
			sumRest += rest;
		}
	...
	async forcastBasedCharging() {
		...
		*/
		// Der surplusMinSoc wird auf die Hälfte zwischen simSoc und surplusBufferSOC gesetzt
		// let surplusMinSoc = (surplusBufferSOC - simSoc) / 2 + simSoc;
		/**
		...
		// Safety offset for surplus SOC calculations (1% margin)
		const SURPLUS_OFFSET = 1;

		// Calculate the minimal surplus SOC with safety margins and ensure it stays within inverter limits.
		const surplusMinSoc = clamp(simSoc + SURPLUS_OFFSET, this.inverter.dischargeCutoffCapacity, this.inverter.chargingCutoffCapacity - SURPLUS_OFFSET);
		await this.inverter.setSurplusMinSoc(surplusMinSoc);
		// Plausibilitäts‑Clamps für den Puffer‑SOC
		surplusBufferSOC = surplusMinSoc + 15; // PufferSOC soll mindestens 15% über simSoc liegen
		surplusBufferSOC = Math.max(surplusBufferSOC, surplusMinSoc);
		surplusBufferSOC = Math.min(surplusBufferSOC, this.inverter.chargingCutoffCapacity);
		if (surplusBufferSOC < surplusMinSoc) surplusBufferSOC = 100;
		...
		if (this.debug === 2) {
			...
			this.adapter.log.debug(`forcastBasedCharging tbl - ${JSON.stringify(tbl)}`);
			this.adapter.log.debug(`forcastBasedCharging ${JSON.stringify(this._getSurplusToday(this.now.idx))}`);
		}
		this.adapter.log.debug(`forcastBasedCharging overSOC ${overSoc} simSoc ${simSoc} surplusMinSoc ${surplusMinSoc} surplusBufferSOC ${surplusBufferSOC}`);
		...
		if (h >= 22) {
			if (this.averageConsumption.value < 150) {
				if (this.inverter.dischargeCutoffCapacity < 20 && this.inverter.dischargeCutoffCapacity < this.SOC) {
					if (await this.inverter.isBatteryRunning()) {
						...
						this.adapter.log.info(`Set dischargeCutoffCapacity to ${newVal}%`);
			...
			if (this.averageConsumption.value > 200 && h >= 4) {
				if (this.inverter.dischargeCutoffCapacity > 5) {
					if (await this.inverter.wakeUp()) {
						this.adapter.log.info('Set dischargeCutoffCapacity to 5%');
