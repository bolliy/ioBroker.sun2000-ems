...
class EMS {
    ...
    */
				...
				if (sort[i].total < highRange[k].threshold && sort[i].idx < highRange[k].start) {
					...
					if (!this.load[hh].LockDisCharging || this.load[hh].LockDisCharging <= 0.75) {
						...
						if (dischargedEnergy > 50) {
							...
							this.load[hh].LockDisCharging += 0.25;
	...
	_putInLowZone(highRange, soc) {
		...
		for (let k = 0; k < highRange.length; k++) {
			...
			for (let i = 0; i < relLP.length; i++) {
				let energy = rest;
				const maxChargePower = Math.round(this.inverter.maximumChargePower / 4); //maximum charge power in a quarter of an hour
				if (energy > maxChargePower) energy = maxChargePower;
				...
				if (energy > 0) {
					...
					if (exist > -1) {
						if (loadPoint[exist].charge + energy > maxChargePower) {
							energy = maxChargePower - loadPoint[exist].charge;
		...
		for (let i = 0; i < loadPoint.length; i++) {
			if (loadPoint[i].charge < 50) {
				const hh = (loadPoint[i].idx / 4) >> 0;
				this.load[hh].chargedEnergy -= loadPoint[i].charge;
		...
		this.adapter.log.debug(`LoadPoint ${JSON.stringify(loadPoint)} Sum ${sum} Rest ${sumRest}`);
		return loadPoint;
	}
	...
	async optimizeCharging() {
		...
		let highPrices = this._searchHighPrices(this.SOC, 0);
	...
	async controlCharging2() {
		...
		const power = this.process?.task.chargePoint.charge;
