...
class EMS {
    ...
    */
	...
	async forcastBasedCharging() {
		...
		if (minSoc > this.inverter.chargingCutoffCapacity - 2) minSoc = this.inverter.chargingCutoffCapacity - 2;
		await this.inverter.setSurplusMinSoc(minSoc);
		//PufferSOC auf minSoc + 10% setzen
		bufferSOC = minSoc + 10;
		if (bufferSOC > this.inverter.chargingCutoffCapacity) bufferSOC = this.inverter.chargingCutoffCapacity;
	...
	async processeing2() {
		...
		*/
		...
		this.adapter.log.debug(`### Aktiver ChargePoint${JSON.stringify(task)} ###`);
