	...
	_putInLowZone(highRange, soc) {
		...
		for (let k = 0; k < highRange.length; k++) {
			...
			for (let i = this.now.idx2 + 1; i < highRange[k].start; i++) {
				...
				if (prices[i].total * standbyFactor < highRange[k].threshold) {
					relLP.push({ idx: i, price: prices[i].total * standbyFactor, standbyFactor: standbyFactor });
				}
			}
			...
			for (let i = 0; i < relLP.length; i++) {
				...
				let energy = rest * relLP[i].standbyFactor;
