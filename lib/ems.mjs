    ...
    */
	_lockDisCharging(load, highRange) {
		...
		for (let i = 0; i < sort.length; i++) {
			const dischargedEnergy = load[sort[i].idx].consumption - load[sort[i].idx].pv - load[sort[i].idx].grid; //Battery Entladung
			//console.log('idx '+sort[i].idx+' last '+last+' pv '+load[sort[i].idx].pv);
			if (!load[sort[i].idx].lockDischarging && dischargedEnergy > 0) {
				load[sort[i].idx].lockDischarging = true;
				gridSum += dischargedEnergy;
			}
			if (gridSum >= sum.energy) break;
		}
		this.adapter.log.debug(`_lockDisCharging ${gridSum}`);
	}
	...
	async optimizeCharging() {
		...
		//Ladetabelle zurücksetzen
		for (let i = this.now.idx + 1; i < this.load.length; i++) {
			if (this.load[i].chargedEnergy) {
				this.load[i].chargedEnergy = 0;
			}
			if (this.load[i].lockDischarging) {
				this.load[i].lockDischarging = false;
			}
		}
