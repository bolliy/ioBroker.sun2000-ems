...
class EMS {
    ...
    */
	...
	async controlCharging2() {
		...
		//
		if (this.SOC < this.process.tast.chargePoint?.soc) {
			...
			if (!isStarted) {
				this.adapter.log.info('Start charging from grid...');
	...
	async dataPolling() {
		...
		this.scheduleHandle2 = cron.schedule('* * * * *', async () => {
			...
			/*
			const state = await this.adapter.getForeignStateAsync('sun2000.0.inverter.0.control.battery.forcibleChargeOrDischarge');
			const isStarted = state && state?.val === 1 && state?.ts >= this.now.date.getTime();
			this.adapter.log.debug(`isStarted ${isStarted} ${state?.ts} >= ${this.now.date.getTime()}`);
			*/
