...
class EMS {
	...
	_putInLowZone(highRange, soc) {
		...
		for (let k = 0; k < highRange.length; k++) {
			const relLP = []; //relevante Ladepunkte
			for (let i = this.now.idx2 + 1; i < highRange[k].start; i++) {
				const standbyFactor = 1 + (highRange[k].threshold - i) / 400; //Stanby-By Verluste ca. +1% pro Stunde
				const newPrice = Math.round(prices[i].total * standbyFactor);
				//günstiger als die untere Preisschwelle
				if (newPrice < highRange[k].threshold) {
					relLP.push({ idx: i, price: newPrice, standbyFactor: standbyFactor });
				}
			}
			...
			for (let i = 0; i < relLP.length; i++) {
				...
				if (energy > 0) rest -= energy / relLP[i].standbyFactor;
				if (rest <= 0) break;
			}
			sumRest += rest;
		}
